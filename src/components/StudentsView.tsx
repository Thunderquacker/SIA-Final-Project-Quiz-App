        ...
        <div className="md:flex md:items-center md:justify-between gap-6 mb-6">
          <div className="flex items-center gap-4 flex-wrap">
            {['All Students', '10A', '10B'].map((tab) => (
              <button
                key={tab}
                className="rounded-full border border-zinc-800 bg-[#0D0D0E] px-4 py-2 text-sm text-white transition hover:border-purple-600"
              >
                {tab}
              </button>
            ))}
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <div className="relative min-w-[220px]">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-zinc-800 bg-[#0D0D0E] py-3 pl-10 pr-4 text-sm text-white outline-none focus:border-purple-600"
              />
            </div>
                ...
                <tr key={student.id} className={index % 2 === 0 ? 'bg-[#121213]' : 'bg-[#0D0D0E]'}>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-purple-600/20 flex items-center justify-center text-sm font-bold text-white">{student.name.split(' ').map((word) => word[0]).join('')}</div>
                      <div>
